...
| math.modf(x) | 向零取整，把参数 x 分为整数和小数，返回两个值 | math.modf(23.45) |
| math.ceil(x) | 向上取整( 向正无穷取整 )为最接近的整数<br> <br> math.ceil(x) == ⌈x⌉ | math.ceil(4.5) |
| math.floor(x) | 向下取整( 向负无穷取整 )为最接近的整数<br> math.floor(x) == ⌊x⌋ | math.floor(4.5) |
...
| math.roundToEven(x) | 改良版银行家舍入法取整 | 使用 aardio 改良版银行家舍入法。<br><br>舍入规则为：尾数四舍六入，五后为零前凑偶（只看一位零）。<br><br>解决问题1: 1 到 9 出现机率一样，5 到 9 进位导致进多舍少。<br>解决问题2: 浮点数误差导致的不合理舍入。<br><br>注意四舍五入非常多的规则，各种语言的默认实现并不完全一致，并且计算机浮点数都存在可容忍的精度误差 |
| <p>数</p> <p>学</p> <p>知</p> <p>识</p> <p>回</p> <p>顾</p> | 对于所有实数 x:<br> <p> x-1 < ⌊x⌋ <= x <= ⌈x⌉ < x+1 <br> <br> 对于所有整数 x:<br> <p>⌊x/2⌋ + ⌈x/2⌉ == x <br><br>对于所有整数 a 和非零整数:<br>a % b == a - ⌊a/b⌋ b <br><blockquote>aardio 取模运算符 `%` 的计算过程与此相同，在计算 ⌊a/b⌋ 这一步时对结果下取整（向负无穷大取整， 注意不同编程语言的实现存在差异， Python 这一步也是向下取整，但 C 语言是向零取整），a % b 取模运算结果的符号与 b 的符号相同。</blockquote> |
